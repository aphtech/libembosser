...
public class GenericTextDocumentHandlerTest {
	...
	public Iterator<Object[]> handlerProvider() {
		...
		data.add(new Object[] {createHandlerBuilder().setLeftMargin(3).setTopMargin(2).build(), multiPageDocumentInput, Arrays.stream(multiPageDocumentOutputStrings).map(s -> String.format("\r\n\r\n   %s%s", s, "\f")).collect(StringBuilder::new, StringBuilder::append, StringBuilder::append).toString().getBytes(Charsets.US_ASCII)});
		// Multiple copy tests

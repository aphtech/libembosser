...
public class GenericTextDocumentHandlerTest {
	...
	public Iterator<Object[]> handlerProvider() {
		...
		data.add(new Object[] {createHandlerBuilder().setEndOfLine(new byte[] {'\n'}).setEndOfPage(new byte[] {'\r', '\n', '\f'}).padWithBlankLines(true).build(), multiLineDocumentInput, String.join("\n", multiLineDocumentOutputString).concat(Strings.repeat("\n", 22)).concat("\r\n\f").getBytes(Charsets.US_ASCII)});
		data.add(new Object[] {createHandlerBuilder().setEndOfLine(new byte[] {'\n'}).setEndOfPage(new byte[] {'\n', '\f'}).padWithBlankLines(true).build(), multiLineDocumentInput, String.join("\n", multiLineDocumentOutputString).concat(Strings.repeat("\n", 23)).concat("\f").getBytes(Charsets.US_ASCII)});
